...
/* 파일 업로드 관련 변수 */
	...
	$(document).ready(function(){
		...
		$("#uploadFile").on("change", function(e){ // e안에 업로드 파일이 들어가있다고생각
			...
			// 파일 배열 담기
			var filesArr = Array.prototype.slice.call(files); // filesArr = Array.prototype.slice.call -> 배열,파일을 담는 방식
			//파일 개수 확인 및 제한
			...
			filesArr.forEach(function (f){
				var reader = new FileReader(); // FileReader -> 파일을 읽어서 데이터화시키는 
	...
	function fileDelete(fileNum){ // 파일삭제
		...
		<form id="saveFrm" name="saveFrm">
			...
			<table>
				...
				<tr>
					<th>첨부파일</th>
					<td>
						<input type="file" class="text" id="uploadFile" name="uploadFile" multiple/> <!-- multiple : 파일 여러건 올릴때 -->
						<div id="boardFileList"></div> <!-- 선택된 파일보여주기 -->
					</td>
				</tr>
			</table>
		</form>
